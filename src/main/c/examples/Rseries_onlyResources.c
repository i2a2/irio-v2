...
void usage(char *name) {
        printf("This example checks the terminals in the FPGA design\n"
                ...
                "If OPEN NI-RIO Linux Version is installed: Use lsrio command to identify the RIO devices included in the fast controller\n"
        		...
        		"\n"
        		"\t<Serial_Number>: Device serial number, uses lsrio/lsni -v command to get it.\n"
        		"\t<FlexRIO_model>: Device model, uses lsrio/lsni -v command to get it.\n"
        		"\n"
...
int main (int argc, char **argv)
{
	...
	asprintf(&NIriomodel,"PXI-%sR",argv[2]);
	asprintf(&filePath,"%s/%s/%s/",(char*)get_current_dir_name(),"resourceTest",argv[2]);

	//-----------------CPUDAQ Test
	msgtest(0,CPUDAQ Test);
	//asprintf(&bitfileName,"Rseries_%s",argv[2]); // Bitfile de Mariano
	asprintf(&bitfileName,"RseriesonlyResources_%s",argv[2]); // Bitfile de Jose Carlos
	printf("Test 0: CPU DAQ.Should Found:\n");

	//TODO: Actualizar recursos esperados cuando tengamos definido qué queremos implementar en LabVIEW
	printf("¡¡¡ACTUALIZAR RECURSOS!!!");
	printf("2 DMAs\n");
	printf("2 AI\n");
	printf("2 AO\n");
	printf("2 auxAI\n");
	...
	myStatus=irio_initDriver("testCPUDAQ",argv[1],NIriomodel,bitfileName,"V1.2",verbosity,filePath,filePath,&p_DrvPvt,&status);
